...
def mind_descriptor_2d_multichannel(
    ...
    Args:
        ...
        offsets: list of (dy,dx) shifts.
            If None, uses 4-neighborhood with given dilation.
...
def mind_loss_2d_multichannel(
    ...
    Args:
        img_a, img_b: (H,W), (C,H,W), or (N,C,H,W).
            Same (N,H,W); C may be same or broadcastable.
    ...
    """
    desc_a = mind_descriptor_2d_multichannel(
        img_a, patch_radius, offsets, dilation, eps
    )
    desc_b = mind_descriptor_2d_multichannel(
        img_b, patch_radius, offsets, dilation, eps
    )
    diff = (desc_a - desc_b).abs()
    if reduction == "mean":
        return diff.mean()
    elif reduction == "sum":
        return diff.sum()
